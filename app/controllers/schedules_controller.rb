class SchedulesController < ApplicationController
  ...
  private
  def set_schedule
    @schedules = Matching.where(host_user_id: current_user.id).or(Matching.where(guest_user_id: current_user.id))
    @unapproved = @schedules.where(guest_user_id: current_user.id).where(status: "申込中").where('scheduled_date>= ?', Date.today)
